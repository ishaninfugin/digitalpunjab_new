...
mysql_query("update orders set read_status='yes'");
...
if (isset($_REQUEST['confirmorder'])) {
    ...
    mysql_query("update orders set deduct='" . $_REQUEST['deduct'] . "' where id=" . $_REQUEST['updateid']);
    $amount_inserted = mysql_query("insert into account (`balance`,`uid`,`detail`,`datee`) values('-" . $_REQUEST['deduct'] . "','" . $_REQUEST['uid'] . "','Deduct By " . $_SESSION['ADMINEMAIL'] . " For Your Order','" . date("d-m-Y") . "')");

    //debit admin balance by requested deduct amount
    if(is_numeric($_REQUEST['deduct'])){
        $new_balance = ( floatval($user_data['msgcredit']) - floatval($_REQUEST['deduct']));
        if ($amount_inserted) {
...
if (isset($_REQUEST['filterconfirmorder'])) {
    ...
    mysql_query("update orders set filterdeduct='" . $_REQUEST['filterdeduct'] . "' where id=" . $_REQUEST['updateid']);
    $amount_inserted = mysql_query("insert into account (`balance`,`uid`,`detail`,`datee`,`filter`) values('-" . $_REQUEST['filterdeduct'] . "','" . $_REQUEST['uid'] . "','Filter Deduct By Admin For Your Order','" . date("d-m-Y") . "',1)");
...
if (isset($_GET['deleteId'])) {
    mysql_query("delete from orders where id=" . $_GET['deleteId']);
}

if (isset($_REQUEST['deleteId'])) {
    mysql_query("delete from orders where id='" . $_REQUEST['deleteId'] . "'  and deduct=''");
}

$OrderList = GetMultiRowsOnCondition('orders', '1 order by id DESC LIMIT 0 , 30');
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                          <div class="widget flat radius-bordered">
							<?php if(!AdminAccess($_SESSION['ADMINID'], array('A','S'))){?>
							
                                    <div class="widget-header bg-blue">
                                        <span class="widget-caption">Looks like you've lost you way. Error 404 - Page not found.</span>
                                    </div>
...
//                    bInfo: false,
                    ...
                    "ajax": {
                        ...
                        type: 'POST',
                        data: {
                            'requestType': 'order_list'
                        }
                    },
//                    "aoColumnDefs": [{ "bVisible": false, "aTargets": [2] }],
                    dom: 'Blfrtip',
                    buttons: [
<?php if(!AdminAccess($_SESSION['ADMINID'], array('S'))){ ?>
                        {
                            text: 'Remove',
                            action: function(e, dt, node, config) {
                                ...
                                if (choice) {
                                    ...
                                    $.ajax({
                                        dataType: 'json',
                                        type: 'post',
                                        data: {requestType: 'remove_order', ids: selected},
                                        url: 'controller.php',
                                        success: function(r) {
                                            if (r.status == 'success') {
                                                oTable.ajax.reload();
...
<?php } ?>
                        {
                            ...
                            action: function(e, dt, node, config) {
                                window.location = 'exportreport.php';
                            }
                        },
						{
                            text: 'Download Duplicate Numbers',
                            action: function(e, dt, node, config) {
                                $.post('getduplicate.php',{ids:selected},function(data){
									if(data != ''){
										window.open($.trim(data),'_blank');
									}
								});
                            }
                        }
					...
					"createdRow": function ( row, data, index ) {
						if ( selected.indexOf(data[2])!=-1 ) {
							$(row).addClass('selected');
						}
					}
                }).on('select',function (e, dt, type, indexes) {
						if ( type === 'row' ) {
       						 var data = oTable.cell(indexes,2).data();
               ...
               // $('.dataTables_scrollHead').attr('style','overflow: auto; position: relative; border: 0px; width: 100%;');
                 //$('.large-table-fake-top-scroll-container-3').attr('style','max-width: 100%;  overflow-x: scroll; overflow-y: auto;');
				// $('.large-table-fake-top-scroll-container-3 div').attr('style','font-size:1px;  line-height:1px;');
...
//            });
            ...
            $('#editable').on('submit', '.Edit_ded', function(e) {
                e.preventDefault();
                var credit = $(this).find('input[name=msgcredit]').val();
            ...
            $('#editable').on('submit', '.Filter_Edit_ded', function(e) {
                e.preventDefault();
                var filtercredit = $(this).find('input[name=filtercredit]').val();
                var data = {requestType: 'filter_deduct_amount', data: $(this).serialize()};
                if (filtercredit) {
                    var conf = confirm('Do you really want to deduct credit?');
                    if (conf) {
                        $.ajax({
