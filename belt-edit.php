...
ob_start();
	...
	$PageTitle = 'Update Belt';
	
	$menuName = 'belt';
	$submenuName = 'belt-list';
	
	####@@@ CHECK BELT ID
	if(isset($_REQUEST['id'])) {
		$BeltId = intval($_REQUEST['id']);
	}
	else {
		echo "<script> window.location.href = 'belt-list.php'; </script>";
		exit();
	}
	
	#### UPDATE BELT CODE
	if(isset($_REQUEST['UpdateBelt'])) {
		$title = $_REQUEST['title'];
		$content = mysql_real_escape_string(trim(strtolower($_REQUEST['content'])));
		
		$data = "title = '$title', content = '$content' "; 
		$condition = "id = '$BeltId' ";
		UpdateRowOnCondition(BELT, $data, $condition);
	}
	
	$BeltDetail = GetSingleRow(BELT, $BeltId);
...
?>
                ...
                <form method="post" class="form-horizontal info">
                    <div class="form-group"><label class="col-sm-2 control-label">Name</label>
                        <div class="col-sm-10">
                        <input type="text" name="title" class="form-control validate[required,custom[onlyLetterSp]]" value="<?php echo $BeltDetail['title']; ?>">
                        </div>
                    </div>
                    <div class="form-group"><label class="col-sm-2 control-label">Remark</label>
                        <div class="col-sm-10">
                        <textarea type="text" name="content" class="form-control"><?php echo $BeltDetail['content']; ?></textarea>
                        </div>
                    </div>
                    <div class="hr-line-dashed"></div>
                    <div class="form-group">
                        <div class="col-sm-4 col-sm-offset-2">
                            <button class="btn btn-primary" name="UpdateBelt" type="submit">Update Belt</button>
                        </div>
                    </div>
                </form>
            </div>
