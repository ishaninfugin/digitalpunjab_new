...
ob_start();
	...
	if(isset($_REQUEST['editId'])) {
		...
		echo "<script> window.location.href = 'sr-edit.php?id=".$editId."'; </script>";
		exit();
	}
	...
	if(isset($_REQUEST['upassId'])) {
		...
		echo "<script> window.location.href = 'sr-upass.php?id=".$upassId."'; </script>";
		exit();
	}
			...
			$sr=1;
				foreach($USerList as $USerDetail) {
                    ...
                    <tr class="gradeX" id="list_<?php echo $USerDetail['id']; ?>">
                        ...
                        <td><?php if($USerDetail['active'] == 1) echo "Active"; else echo "Deactive"; ?></td>
                        <td class="center">
                        <a href="sr-list.php?editId=<?php echo $USerDetail['id']; ?>" class="btn btn-success btn-xs">Edit</a>
                        <a href="sr-list.php?upassId=<?php echo $USerDetail['id']; ?>" class="btn btn-success btn-xs">Update Password</a>
                        <a onClick="DeleteRecord('<?php echo ADMIN; ?>', <?php echo $USerDetail['id']; ?>)" class="btn btn-danger btn-xs">Delete</a>
                        </td>
                    </tr>
                    <?php
				}
